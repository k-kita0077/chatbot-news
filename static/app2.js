...
function handleStartButtonClick() {
    ...
    recognition.onresult = event => {
        ...
        if (text.indexOf('天気') !== -1) {
            ...
            // ニュース以外はわからないよ〜.
            let synthes = new SpeechSynthesisUtterance('ごめんなさい、天気以外はわかりません');
            synthes.lang = "ja-JP";
 ...
 */
function showGetWeather() {
    api('/api/get_weather').then(response => {
        console.log("getweather");
        let { content } = JSON.parse(response);
        console.log(content);

        content = content.split("-")[0];
        content = `明日の天気は${content}です`
...
function startIntro() {
    ...
    return new Promise((resolve, reject) => {
        ...
        let texts = "「明日の天気を教えて」と聞いてみてください。".split('');
